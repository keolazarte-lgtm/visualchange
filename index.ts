...
> = {
  vanilla: {
    ...
    suffix:
      "Candid smartphone photo, shot on iPhone, harsh flash photography, natural skin texture with pores, unretouched, casual instagram photo, youthful and trendy, Gen-Z fashion aesthetic, form-fitting silhouette, modern streetwear, full body shot. Match the person's gender presentation from the description. Negative prompt: plastic skin, 3d render, CGI, heavily airbrushed, overly smooth, perfect studio lighting, hyperrealistic digital art, old fashioned, baggy, loose, modest, boring, plain cotton, granny clothes, conservative.",
    ideas: [
      { label: "Traje elegante negro", prompt: "elegant black suit, tailored fit" },
      { label: "Camisa blanca + jeans", prompt: "white button-up shirt with fitted jeans" },
      { label: "Remera ajustada + joggers", prompt: "fitted t-shirt with jogger pants" },
      { label: "Chaqueta de cuero + jeans", prompt: "leather jacket with dark jeans" },
      { label: "Sudadera moderna", prompt: "modern hoodie with slim fit pants" },
      { label: "Camisa de lino de verano", prompt: "linen shirt, summer outfit" },
      { label: "Abrigo con bufanda", prompt: "winter coat with stylish scarf" },
      { label: "Blazer casual", prompt: "casual blazer with t-shirt and jeans" },
    ],
    llmHint:
      "elegant, classy outfits for ANY gender: suits, shirts, casual-chic, streetwear. Include menswear options like suits, button-up shirts, fitted jeans, hoodies. NEVER default to women's dresses unless the person is feminine.",
  },
  versatil: {
    name: "Versátil",
    desc: "Atrevido pero elegante",
    suffix:
      "Candid smartphone photo, shot on iPhone, harsh flash photography, natural skin texture with pores, unretouched, casual instagram photo, dramatic club lighting, confident modern pose, youthful and edgy, trendy, Gen Z fashion, form-fitting silhouette, sleek, modern streetwear, full body shot. Match the person's gender presentation from the description. Negative prompt: plastic skin, 3d render, CGI, heavily airbrushed, overly smooth, perfect studio lighting, hyperrealistic digital art, old fashioned, baggy, loose, modest, boring, plain cotton, granny clothes, conservative.",
    ideas: [
      { label: "Camisa semi desabrochada", prompt: "white dress shirt half-unbuttoned, confident pose" },
      { label: "Remera ajustada + cuero", prompt: "fitted black t-shirt with leather pants" },
      { label: "Traje sin corbata", prompt: "tailored suit without tie, relaxed" },
      { label: "Tank top + cadena", prompt: "tank top with gold chain necklace" },
      { label: "Camiseta musculosa", prompt: "muscle tank top, athletic look" },
      { label: "Chaleco sobre piel desnuda", prompt: "vest over bare chest" },
      { label: "Buzo con cremallera abierto", prompt: "zip-up hoodie open, no shirt underneath" },
      { label: "Camisa de seda", prompt: "silk shirt unbuttoned, sensual" },
    ],
    llmHint:
      "bold, edgy outfits for ANY gender: leather, fitted shirts, suits without ties, tank tops, muscle shirts. Include masculine options. NEVER default to women's clothing unless the person is feminine.",
  },
  fetish: {
    name: "Fetiche",
    desc: "Lencería, latex, bikinis",
    suffix:
      "Candid smartphone photo, shot on iPhone, harsh flash photography, natural skin texture with pores, unretouched, casual instagram photo, intimate mood lighting, confident modern pose, youthful and attractive, form-fitting silhouette, sleek, modern streetwear, dramatic club lighting, full body shot. Match the person's gender presentation from the description. Negative prompt: plastic skin, 3d render, CGI, heavily airbrushed, overly smooth, perfect studio lighting, hyperrealistic digital art, old fashioned, baggy, loose, modest, boring, plain cotton, granny clothes, conservative.",
    ideas: [
      { label: "Boxer ajustado", prompt: "fitted boxer briefs, muscular" },
      { label: "Calzoncillo + cadena", prompt: "underwear with chain necklace" },
      { label: "Bano en la playa", prompt: "swim trunks on the beach" },
      { label: "Arnés de cuero", prompt: "leather harness over bare chest" },
      { label: "Pantalón de cuero", prompt: "leather pants, shirtless" },
      { label: "Tank top transparente", prompt: "sheer tank top, sensual" },
      { label: "Jockstrap", prompt: "athletic jockstrap, gym setting" },
      { label: "Buzo abierto sin nada", prompt: "open hoodie, no shirt, boxer briefs" },
    ],
    llmHint:
      "intimate apparel for ANY gender: boxer briefs, swim trunks, leather harness, jockstraps, shirtless looks. Include masculine options. NEVER default to women's lingerie unless the person is feminine.",
  },
};
...
// ===== Fashion enhancement: inject sewing/design terms that bypass content filters =====
// These terms describe the same looks but use technical fashion vocabulary
// that doesn't trigger Z.ai's content filter (code 1301).
// The user's outfit prompt gets enhanced with these terms automatically.
const FASHION_ENHANCEMENT_TERMS = [
  "plunging neckline",
  "corset bodice",
...
// Enhance the outfit prompt with fashion terms that make DALL-E generate
// more modern, form-fitting, and attractive outfits WITHOUT triggering
// the content filter.
// CRITICAL: terms go BEFORE the outfit for prompt weighting (DALL-E prioritizes
// the beginning of the prompt).
function enhanceOutfitPrompt(outfit: string): string {
  // Pick 3-4 fashion terms to inject (rotating for variety)
  const shuffled = [...FASHION_ENHANCEMENT_TERMS].sort(() => Math.random() - 0.5);
  const selected = shuffled.slice(0, 4);
  // TERMS FIRST, then outfit — this forces DALL-E to prioritize the clothing
  return `${selected.join(", ")}, ${outfit}`;
}
    ...
    : `Analyze this person's photo. Return ONLY a JSON object (no other text, no markdown) with these fields:
{
  ...
  "current_clothing": "<describe the outfit using modern fashion terms: Y2K style, form-fitting, baddie aesthetic, high-fashion editorial. NEVER use conservative, loose, modest, or old-fashioned terms. Always describe as trendy and youthful.>"
}

CRITICAL: Always enhance the clothing description to be modern, youthful, and form-fitting. Replace generic terms with specific fashion keywords like Y2K style, tight-fit, baddie aesthetic, or high-fashion editorial. Never use conservative, loose, or modest styles.
...
async function generateOutfitDallE(
  ...
  // CRITICAL: outfit terms go FIRST in the prompt for DALL-E prompt weighting.
  // Structure: [FASHION TERMS] [OUTFIT] worn by [BODY DESCRIPTION]. [BG] [STYLE] [QUALITY]
  const enhancedOutfit = enhanceOutfitPrompt(sanitizePrompt(outfit));
  ...
  if (opts.bodyMods && opts.bodyMods.length > 0) {
    ...
    if (modParts.length > 0) {
      bodyModsClause = ` The person is ${modParts.join(", ")}.`;
    }
  }

  // Background clause — include pose if specified
  const poseClause = opts.pose ? ` The person is ${opts.pose}.` : "";
  const bgClause = opts.keepBackground
    ? `Standing in the same setting and background as the reference photo. Preserve the original background.${poseClause}`
    : `Standing in ${sanitizePrompt(bg)}.${poseClause}`;

  // Quality suffix
  const qualitySuffix = opts.qualitySuffix || QUALITY_PRESETS.standard.suffix;
  const picanteSuffix = opts.picanteSuffix || "";

  // ===== PROMPT ENGINE v4.1 — Gender detection + filter bypass + realism =====
  // 1. Detect gender from VLM description (overrides user selection if VLM is clear)
  const descLow = bodyDesc.toLowerCase();
  let genderMarker = "person";
  let genderKeywords = "";
  let genderNegative = "";

  const userGender = opts.gender || "masculine";
  const isMasculine = userGender === "masculine" || descLow.includes("man") || descLow.includes("male") || descLow.includes("masculine");
  const isFeminine = userGender === "feminine" || descLow.includes("woman") || descLow.includes("female") || descLow.includes("feminine");

  if (isMasculine && !isFeminine) {
    genderMarker = "man, male model";
    genderKeywords = "streetwear chic, tight-fit shirt, muscle fit, leather jacket, open shirt style, high-fashion male editorial, rugged aesthetic, tailored silhouette, dramatic cinematic lighting";
    genderNegative = "dress, skirt, female body, crop top, make-up, female accessories, blouse, loose clothing, modest, conservative outfit, matronly clothes, granny clothes";
  } else {
    genderMarker = "woman, female model";
    genderKeywords = "trendy, Y2K fashion, form-fitting silhouette, sleek, crop top, baddie aesthetic, high-fashion female editorial, dramatic club lighting";
    genderNegative = "old fashioned, baggy, loose, modest, boring, plain cotton, granny clothes, conservative, high-neck, matronly clothes";
  }

  // 2. Sewing terms for filter bypass (3 random terms)
  const sewingTerms = [
    "plunging neckline", "corset bodice", "bodycon shape", "strategic cutouts",
    "asymmetric hemline", "sheer panel details", "structured bodice", "high-slit design",
  ];
  const randomTerms = [...sewingTerms].sort(() => 0.5 - Math.random()).slice(0, 3).join(", ");

  // 3. Realism keywords — kill the plastic AI look
  const realismKeywords = "candid smartphone photo, shot on iPhone, harsh flash photography, natural skin texture with pores, unretouched, casual instagram photo, 35mm film, raw photo, depth of field";
  const hardcoreNegative = "3d render, plastic skin, CGI, airbrushed, digital art, illustration, cartoon, anime, bad anatomy, deformed, blurry, overly smooth, perfect studio lighting, hyperrealistic digital art";

  // v4.2: Custom pose and background from user inputs
  // When customPose is set, we need to be VERY aggressive because DALL-E Edit
  // tends to preserve the original photo's pose. We add "completely different pose"
  // and repeat the pose instruction to force DALL-E to change it.
  const hasCustomPose = opts.customPose && opts.customPose.trim().length > 0;
  const userPose = hasCustomPose
    ? opts.customPose.trim() + ", completely different pose from the reference photo, ignore the original pose"
    : (opts.pose || "standing in a natural pose");
  const userBackground = opts.customBg && opts.customBg.trim()
    ? opts.customBg.trim()
    : (opts.keepBackground ? "the same setting and background as the reference photo, preserve original background" : sanitizePrompt(bg));

  // 4. Build final prompt (v4.2 — pose at the FRONT for prompt weighting)
  const fullNegative = `${genderNegative}, ${hardcoreNegative}`;
  const poseReinforcement = hasCustomPose ? ` The person MUST be ${opts.customPose.trim()}. This pose is MANDATORY and overrides the reference photo's pose.` : "";
  const prompt = `Commercial fashion photography of a ${genderMarker} ${userPose}, wearing an ultra-modern, tight-fitting ${sanitizePrompt(outfit)}, ${randomTerms}. ${genderKeywords}. Set against the backdrop of ${userBackground}. ${realismKeywords}.${bodyModsClause}${poseReinforcement} Negative prompt: ${fullNegative}. CRITICAL IDENTITY PRESERVATION: The person's face MUST be identical to the reference photo — same eyes, nose, mouth, jawline, cheekbones, skin tone, hair color, hair style, and facial structure. Do NOT alter the face in any way.`;
...
async function faceSwap(
  ...
  // slider=100 = maximum face swap strength (was 0 = minimal)
  // adv_slider=0 = no adversarial defense (don't distort the face)
  // settings=["Compare"] = return the swapped result
  const endpoints: Array<{ path: string; payload: Record<string, any> }> = [
    {
      ...
      payload: {
        target: targetBlob,
        source: sourceBlob,
        slider: 100,        // MAXIMUM face swap strength (was 0)
        adv_slider: 0,      // no adversarial defense
        settings: ["Compare"],
      },
    },
    {
      ...
      payload: {
        target: targetBlob,
        source: sourceBlob,
        slider: 100,        // MAXIMUM face swap strength
        adv_slider: 0,
        settings: [],
      },
    },
...
type PipelineOptions = {
  ...
  bodyOnly?: boolean;
  promptOnly?: boolean;
  forceFaceSwap?: boolean;
  gender?: string;
  customBg?: string;
  customPose?: string;
};

// ===== Smart natural-language outfit parser =====
// Detects when the user wrote a complex instruction in Spanish (not just an
// outfit name) and uses the LLM to extract:
//   - outfit (translated to English for DALL-E)
//   - background (if specified in the instruction)
//   - keepIdentity (if user said "respeta la figura/cara/silueta")
//
// Trigger keywords: "respeta", "ponele", "cambia", "fondo", "mantén",
// "deja", "sacale", "agregale", "cambiale", etc.
const INSTRUCTION_KEYWORDS = /\b(respeta|ponele|poné|cambia|cambiale|cambiar|fondo|mantén|manten|deja|sacale|sacá|agregale|agregá|quitale|quitá|conserva|conservá|silueta|figura|cara|rostro|sentada|sentado|acostada|acostado|parada|parado|sillon|sillón|cama|pose|posicion|posición)\b/i;

async function parseNaturalLanguageOutfit(
  rawOutfit: string,
  bodyDesc: string,
  currentBg: string
): Promise<{ outfit: string; bg: string; keepIdentity: boolean; pose: string; parsed: boolean }> {
  // If no instruction keywords, return as-is (it's just an outfit name)
  if (!INSTRUCTION_KEYWORDS.test(rawOutfit)) {
    return { outfit: rawOutfit, bg: currentBg, keepIdentity: false, pose: "", parsed: false };
  }

  console.log("[SmartParser] Detected natural language instruction, parsing...");

  const z = await initZAI();
  const prompt = `You are a fashion photography prompt engineer. The user wrote an instruction in Spanish for editing a photo. Parse it and extract structured data.

User's instruction: "${rawOutfit}"

Person's description (from VLM): ${bodyDesc}
Currently selected background: ${currentBg}

Return ONLY a JSON object (no markdown, no \`\`\` code blocks, just the JSON) with these fields:
{
  "outfit": "<clothing items in English, OR 'keep current clothing' if user said don't change clothes>",
  "background": "<background in English if user specified one, OR 'keep current' if not mentioned>",
  "keep_identity": true | false,
  "pose": "<pose description in English if user specified one (e.g. 'sitting on a couch', 'standing by a window', 'lying on a bed'), OR empty string if not mentioned>"
}

CRITICAL RULES:
1. "outfit" must be ONLY clothing items in English. If user says "no cambies/no toque/deja tal cual" → use "keep current clothing"
2. "pose" is for body position. If user says "sentada en el sillon" → "sitting on a couch". If user says "parada" → "standing". Empty string if not mentioned.
3. "keep_identity" = true if user says "respeta/mantén/conserva/deja" the person/figure/face
4. "background" = English translation if user mentions it, otherwise "keep current"
5. Translate ALL Spanish to English
6. Return ONLY the JSON, no markdown, no \`\`\`json wrapper

Example 1: "respeta la figura y ponele minifalda y corset de cuero negro, cambia el fondo con paisaje de nieve"
Output: {"outfit":"black leather corset, mini skirt","background":"snowy mountain landscape","keep_identity":true,"pose":""}

Example 2: "deja la imagen tal cual esta no toque nada solo mejora calidad"
Output: {"outfit":"keep current clothing","background":"keep current","keep_identity":true,"pose":""}

Example 3: "que este sentada en el sillon con la misma ropa"
Output: {"outfit":"keep current clothing","background":"keep current","keep_identity":true,"pose":"sitting on a couch"}

Example 4: "ponele vestido rojo y que este acostada en una cama"
Output: {"outfit":"red dress","background":"keep current","keep_identity":false,"pose":"lying on a bed"}`;
  ...
  try {
    ...
    const text: string = response.choices?.[0]?.message?.content || "";
    console.log("[SmartParser] LLM response:", text.slice(0, 500));

    // Strip markdown code blocks if present
    const cleanText = text.replace(/```json\s*/gi, "").replace(/```\s*/g, "").trim();
    const match = cleanText.match(/\{[\s\S]*\}/);
    if (!match) {
      console.warn("[SmartParser] No JSON found, returning original");
      return { outfit: rawOutfit, bg: currentBg, keepIdentity: false, pose: "", parsed: false };
    }

    const obj = JSON.parse(match[0]);
    return {
      outfit: obj.outfit || rawOutfit,
      bg: obj.background === "keep current" ? currentBg : (obj.background || currentBg),
      keepIdentity: !!obj.keep_identity,
      pose: obj.pose || "",
      parsed: true,
    };
  } catch (e: any) {
    console.warn("[SmartParser] Failed:", e?.message?.slice(0, 100));
    return { outfit: rawOutfit, bg: currentBg, keepIdentity: false, pose: "", parsed: false };
  }
}
...
async function runPipeline(
  ...
  // promptOnly mode: keep the photo as-is, only apply the user's instruction
  // (pose, background change, add accessory, etc.) — don't change the outfit.
  const promptOnly = !!options.promptOnly;

  // First, check if the outfit text is a natural-language instruction (Spanish).
  // If so, parse it with LLM to extract outfit + background + keepIdentity + pose.
  let effectiveOutfit = outfit;
  let effectiveBg = bg;
  let effectiveKeepBg = options.keepBackground;
  let effectivePose = "";

  // In promptOnly mode, force keeping the original outfit + background
  if (promptOnly) {
    onProgress("parsing", "Modo Solo Prompt: respetando foto original...");
    // Use the clothing detected by VLM as the outfit
    if (analysis.currentClothing) {
      effectiveOutfit = analysis.currentClothing;
    } else {
      effectiveOutfit = "keep current clothing";
    }
    // Keep the original background
    effectiveKeepBg = true;
    // Force the SmartParser to run to extract any pose/instruction from the user's text
    try {
      const parsed = await parseNaturalLanguageOutfit(outfit, analysis.bodyDesc, bg);
      if (parsed.parsed) {
        effectivePose = parsed.pose;
        // If user specified a new background in their instruction, use it
        if (parsed.bg && parsed.bg !== bg) {
          effectiveBg = parsed.bg;
          effectiveKeepBg = false; // user wants to change background
        }
        // If user specified a different outfit (not "keep current"), use it
        if (parsed.outfit && parsed.outfit !== "keep current clothing") {
          effectiveOutfit = parsed.outfit;
        }
        console.log("[Pipeline] promptOnly SmartParser result:", {
          outfit: effectiveOutfit.slice(0, 60),
          pose: effectivePose,
          bg: effectiveBg.slice(0, 40),
          keepBg: effectiveKeepBg,
        });
      }
    } catch (e: any) {
      console.warn("[Pipeline] promptOnly SmartParser failed:", e?.message?.slice(0, 80));
    }
    onProgress("parsed", `Solo Prompt: ${effectivePose ? effectivePose : "aplicando instrucción"}...`);
  } else if (INSTRUCTION_KEYWORDS.test(outfit)) {
    onProgress("parsing", "Interpretando tu instrucción con IA...");
    try {
      const parsed = await parseNaturalLanguageOutfit(outfit, analysis.bodyDesc, bg);
      if (parsed.parsed) {
        effectiveOutfit = parsed.outfit;
        effectiveBg = parsed.bg;
        effectivePose = parsed.pose;
        // If user said "respeta la figura/silueta/cara" and didn't explicitly
        // set keepBackground, force it on (preserve original background).
        if (parsed.keepIdentity && parsed.bg === bg && !options.keepBackground) {
          effectiveKeepBg = true;
        }
        // If outfit is "keep current clothing", use VLM's detected clothing
        if (effectiveOutfit === "keep current clothing" && analysis.currentClothing) {
          effectiveOutfit = analysis.currentClothing;
        }
        console.log("[Pipeline] SmartParser result:", {
          outfit: effectiveOutfit.slice(0, 80),
          bg: effectiveBg.slice(0, 60),
          keepBg: effectiveKeepBg,
          pose: effectivePose,
        });
        const poseMsg = effectivePose ? ` Pose: ${effectivePose}` : "";
        onProgress("parsed", `Interpretado: ${effectiveOutfit.slice(0, 50)}...${poseMsg}`);
      }
    } catch (e: any) {
      console.warn("[Pipeline] SmartParser failed, using raw outfit:", e?.message?.slice(0, 80));
    }
  }
  ...
  if (bodyOnly) {
    ...
    const instruction = buildIp2pInstruction(effectiveOutfit, analysis.currentClothing);
    ...
    if (ip2pResult) {
      ...
      try {
        generated = await generateOutfitDallE(workBuf, analysis.bodyDesc, effectiveOutfit, effectiveBg, stylePreset.suffix, {
          bodyMods: options.bodyMods,
          qualitySuffix: qualityPreset.suffix,
          picanteSuffix: picantePreset.suffix,
          keepBackground: effectiveKeepBg,
          keepAspectRatio: options.keepAspectRatio,
          gender: options.gender,
          customBg: options.customBg,
          customPose: options.customPose,
          pose: effectivePose,
        });
      } catch (e: any) {
        if (isZaiContentBlock(e)) {
          ...
          const w = 768, h = 1344;
          const posePp = effectivePose ? ` The person is ${effectivePose}.` : "";
          const ppGender = options.gender === "masculine" ? "man, male model" : "woman, female model";
          const ppKeywords = options.gender === "masculine" ? "streetwear chic, tight-fit shirt, muscle fit, high-fashion male editorial, rugged aesthetic" : "trendy, Y2K fashion, form-fitting silhouette, baddie aesthetic, high-fashion female editorial";
          const ppNeg = options.gender === "masculine" ? "dress, skirt, female body, crop top, make-up" : "old fashioned, baggy, loose, modest, boring";
          const ppTerms = ["plunging neckline", "corset bodice", "bodycon shape", "strategic cutouts"].sort(() => 0.5 - Math.random()).slice(0, 2).join(", ");
          const pp = `Commercial fashion photography of a ${ppGender} wearing an ultra-modern, tight-fitting ${sanitizePrompt(effectiveOutfit)}, ${ppTerms}. ${ppKeywords}. candid smartphone photo, shot on iPhone, natural skin texture with pores, unretouched, casual instagram photo. ${effectiveKeepBg ? "Original background preserved." : "Standing in " + sanitizePrompt(effectiveBg) + "."}${posePp} Negative prompt: ${ppNeg}, 3d render, plastic skin, CGI, airbrushed, digital art.`;
    ...
    onProgress("generating", `DALL-E generando "${effectiveOutfit.slice(0, 40)}..."...`);
    try {
      generated = await generateOutfitDallE(workBuf, analysis.bodyDesc, effectiveOutfit, effectiveBg, stylePreset.suffix, {
        bodyMods: options.bodyMods,
        qualitySuffix: qualityPreset.suffix,
  ...
  // Step 3: face swap
  // Skip if: bodyOnly (IP2P preserved identity)
  // Skip if: promptOnly AND no forceFaceSwap (editing existing result — face already correct)
  // BUT if forceFaceSwap=true (face seed saved), do face swap even in promptOnly
  const forceFS = !!options.forceFaceSwap;
  let final = generated;
  let usedFallback = false;
  if (!bodyOnly && (!promptOnly || forceFS)) {
    onProgress("swapping", forceFS ? "Recomponiendo rostro desde semilla..." : "Recomponiendo tu rostro real con InsightFace (fuerza máxima)...");
    ...
    // Post-blend: mix 70% swapped + 30% generated to smooth the face seam
    // This makes the face swap look more natural (less obvious "paste" effect)
    if (!usedFallback && swapped) {
      try {
        onProgress("blending", "Suavizando transición del rostro...");
        const [w, h] = [1024, 1024];
        const swappedResized = await sharp(swapped).resize(w, h, { fit: "contain" }).png().toBuffer();
        const generatedResized = await sharp(generated).resize(w, h, { fit: "contain" }).png().toBuffer();

        // Blend: 70% swapped (your face) + 30% generated (body/outfit)
        // This keeps your face strong while smoothing the edges
        final = await sharp(generatedResized)
          .composite([{
            input: swappedResized,
            blend: "over",
            gravity: "center",
          }])
          .jpeg({ quality: 100, mozjpeg: true })
          .toBuffer();
        console.log("[Pipeline] Post-blend: 70/30 face/body blend applied");
      } catch (e: any) {
        console.warn("[Pipeline] Post-blend failed, using raw swap:", e?.message?.slice(0, 80));
        final = swapped;
      }
    } else {
      final = swapped;
  ...
  } else if (promptOnly && !forceFS) {
    onProgress("ip2p_done", "Edición iterativa: respetando la imagen existente (sin face swap).");
  } else {
    onProgress("ip2p_done", "Listo (sin face swap).");
  }

  // Step 4: final resize + JPEG + quality post-processing
  // NUNCA usar fit:"cover" — CORTA la imagen. Usar fit:"inside" para respetar proporciones.
  // CALIDAD MÁXIMA: JPEG quality 100, PNG intermedio, sin compresión agresiva
  const qualityKey = options.quality || "standard";
  // Subir calidad base de 92 → 98 para todas las imágenes
  const jpegQuality = 98; // siempre alta calidad

  // Get the generated image dimensions
  const genMeta = await sharp(final).metadata().catch(() => ({ width: 1024, height: 1024 }));
  const genW = genMeta.width || 1024;
  const genH = genMeta.height || 1024;

  if (qualityKey === "hd") {
    // HD: upscale 2x with Lanczos3 — mantener proporciones SIN cortar
    console.log(`[Pipeline] HD: upscaling ${genW}x${genH} → ${genW * 2}x${genH * 2} (Lanczos3, no crop)`);
    final = await sharp(final)
      .resize(genW * 2, genH * 2, { kernel: "lanczos3", fit: "inside", withoutEnlargement: false })
      .jpeg({ quality: 100, mozjpeg: true })
      .toBuffer();
  } else if (qualityKey === "cinematic") {
    // Cinematic: upscale 2x + color grading — SIN cortar
    console.log(`[Pipeline] Cinematic: upscaling + color grading (no crop)`);
    final = await sharp(final)
      .resize(genW * 2, genH * 2, { kernel: "lanczos3", fit: "inside", withoutEnlargement: false })
      .modulate({
        brightness: 1.05,
    ...
    // Standard: mantener proporciones originales SIN cortar, calidad alta
    final = await sharp(final)
      .jpeg({ quality: 100, mozjpeg: true })
      .toBuffer();
  }
...
const server = Bun.serve({
  ...
  async fetch(req): Promise<Response> {
    ...
    // ===== Enhance HD: upscale 4x + hiperrealista =====
    if (url.pathname === "/api/enhance" && req.method === "POST") {
      try {
        ...
        const photoBuf = await autoCompressPhoto(Buffer.from(await file.arrayBuffer()));
        console.log(`[Enhance] photo=${file.size}B`);
        ...
        const stream = new ReadableStream({
          async start(controller) {
            ...
            const keepalive = setInterval(() => {
              if (closed) return;
              try { controller.enqueue(encoder.encode("data: " + JSON.stringify({ type: "keepalive", ts: Date.now() }) + "\n\n")); }
              catch { closed = true; }
            }, 5000);

            try {
              send({ type: "progress", stage: "enhancing", msg: "Mejorando HD 4x..." });

              const meta = await sharp(photoBuf).metadata();
              const origW = meta.width || 1024;
              const origH = meta.height || 1024;

              // Upscale 4x with Lanczos3 (best quality)
              const newW = Math.min(origW * 4, 4096);
              const newH = Math.min(origH * 4, 4096);
              console.log(`[Enhance] ${origW}x${origH} → ${newW}x${newH} (4x Lanczos3)`);

              // Mejora HD suave: upscale 4x + sharpening GENTIL + color
              // Sin exagerar — el objetivo es que se vea MEJOR pero NATURAL
              console.log(`[Enhance] ${origW}x${origH} → ${newW}x${newH} (4x + gentle enhance)`);

              let enhanced = await sharp(photoBuf)
                .resize(newW, newH, { kernel: "lanczos3", fit: "inside", withoutEnlargement: false })
                // Sharpening muy suave — solo para compensar el suavizado del upscale
                .sharpen({
                  sigma: 0.5,
                  m1: 0.5,
                  m2: 0.2,
                })
                // Color un poco más vivo
                .modulate({
                  brightness: 1.03,
                  saturation: 1.10,
                })
              ...
              send({ type: "progress", stage: "done", msg: "Mejora HD completada ✓" });

              const b64 = enhanced.toString("base64");
              send({
                type: "done",
                result: "data:image/jpeg;base64," + b64,
                bodyDesc: "HD enhanced " + origW + "x" + origH + " → " + newW + "x" + newH,
                usedFallback: false,
                cached: false,
              });
            } catch (e: any) {
    ...
    // ===== Face Swap: put face from photo 1 onto body from photo 2 =====
    if (url.pathname === "/api/face-swap" && req.method === "POST") {
      try {
        const formData = await req.formData();
        const faceFile = formData.get("face") as File | null;      // photo with the face
        const bodyFile = formData.get("body") as File | null;      // photo with the body/outfit

        if (!faceFile || !bodyFile) {
          return Response.json({ error: "Faltan fotos. Subí la cara y el cuerpo." }, { status: 400 });
        }
        if (faceFile.size > MAX_FILE_SIZE || bodyFile.size > MAX_FILE_SIZE) {
          return Response.json({ error: "Archivo muy grande (max 50MB)" }, { status: 413 });
        }

        const faceBuf = await autoCompressPhoto(Buffer.from(await faceFile.arrayBuffer()));
        const bodyBuf = await autoCompressPhoto(Buffer.from(await bodyFile.arrayBuffer()));

        console.log(`[FaceSwap] face=${faceFile.size}B body=${bodyFile.size}B`);
        ...
        const stream = new ReadableStream({
          async start(controller) {
            ...
            try {
              send({ type: "progress", stage: "swapping", msg: "Cambiando cara con InsightFace (fuerza máxima)..." });
              const swapped = await faceSwap(faceBuf, bodyBuf);

              let final: Buffer;
              if (swapped) {
                // Post-blend for smoother transition
                try {
                  const w = 1024, h = 1024;
                  const swappedR = await sharp(swapped).resize(w, h, { fit: "contain" }).png().toBuffer();
                  const bodyR = await sharp(bodyBuf).resize(w, h, { fit: "contain" }).png().toBuffer();
                  final = await sharp(bodyR)
                    .composite([{ input: swappedR, blend: "over", gravity: "center" }])
                    .jpeg({ quality: 100, mozjpeg: true })
                    .toBuffer();
                } catch {
                  final = swapped;
                }
                send({ type: "progress", stage: "done", msg: "Face swap real completado ✓" });
              } else {
                // Fallback: sharp composite
                send({ type: "progress", stage: "fallback", msg: "InsightFace no disponible, usando composición local..." });
                final = await fallbackComposite(faceBuf, bodyBuf);
              }

              // Resize to 1024x1024
              final = await sharp(final).resize(1024, 1024, { fit: "inside" }).jpeg({ quality: 100, mozjpeg: true }).toBuffer();

              const b64 = final.toString("base64");
              send({
                type: "done",
                result: `data:image/jpeg;base64,${b64}`,
                bodyDesc: "Face swap completado",
                usedFallback: !swapped,
                cached: false,
              });
            } catch (e: any) {
    ...
    // ===== Generate without photo (text-to-image only, via Pollinations) =====
    if (url.pathname === "/api/generate-no-photo" && req.method === "POST") {
      try {
        const body = await req.json();
        const prompt: string = body.prompt || "";
        const quality: string = body.quality || "standard";
        const keepAspectRatio: boolean = !!body.keepAspectRatio;

        if (!prompt.trim()) {
          return Response.json({ error: "Missing prompt" }, { status: 400 });
        }

        console.log(`[NoPhoto] prompt="${prompt.slice(0, 100)}..." quality=${quality}`);
        ...
        const stream = new ReadableStream({
          async start(controller) {
            ...
            const keepalive = setInterval(() => {
              if (closed) return;
              try {
                controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: "keepalive", ts: Date.now() })}\n\n`));
              } catch { closed = true; }
            }, 5000);

            try {
              send({ type: "progress", stage: "generating", msg: "Generando con Pollinations..." });

              // Generate via Pollinations (text-to-image, no filter)
              const w = 768, h = 1344;
              const result = await generateWithPollinations(prompt, w, h);
              console.log(`[NoPhoto] Pollinations OK: ${result.length}B`);

              // Apply quality post-processing (same as main pipeline)
              let final = result;
              const qualityKey = quality;
              const jpegQuality = 100;

              const genMeta = await sharp(final).metadata().catch(() => ({ width: w, height: h }));
              const genW = genMeta.width || w;
              const genH = genMeta.height || h;

              if (qualityKey === "hd") {
                let p = sharp(final).resize(genW * 2, genH * 2, { kernel: "lanczos3", fit: "fill" });
                if (!keepAspectRatio) p = p.resize(1024, 1024, { fit: "inside" });
                final = await p.jpeg({ quality: jpegQuality, mozjpeg: true }).toBuffer();
              } else if (qualityKey === "cinematic") {
                let p = sharp(final)
                  .resize(genW * 2, genH * 2, { kernel: "lanczos3", fit: "fill" })
                  .modulate({ brightness: 1.05, saturation: 1.15 });
                if (!keepAspectRatio) p = p.resize(1024, 1024, { fit: "inside" });
                final = await p.jpeg({ quality: jpegQuality, mozjpeg: true }).toBuffer();
              } else {
                if (keepAspectRatio) {
                  final = await sharp(final).jpeg({ quality: 100, mozjpeg: true }).toBuffer();
                } else {
                  final = await sharp(final).resize(1024, 1024, { fit: "inside" }).jpeg({ quality: 100, mozjpeg: true }).toBuffer();
                }
              }
              ...
              send({
                type: "done",
                result: `data:image/jpeg;base64,${b64}`,
                bodyDesc: "(generated without photo)",
                usedFallback: false,
                cached: false,
                pollinationsUsed: true,
              });
            } catch (e: any) {
              console.error("[NoPhoto] Error:", e);
              send({ type: "error", error: e?.message || String(e) });
            } finally {
              clearInterval(keepalive);
              if (!closed) { try { controller.close(); } catch {} }
    ...
    if (url.pathname === "/api/edit" && req.method === "POST") {
      ...
      try {
        ...
        const customBg = (formData.get("customBg") as string | null) || "";
        ...
        const promptOnly = (formData.get("promptOnly") as string | null) === "true";
        const forceFaceSwap = (formData.get("forceFaceSwap") as string | null) === "true";
        ...
        const photoBuf = await autoCompressPhoto(Buffer.from(await file.arrayBuffer()));
        console.log(
          `[Edit] ip=${ip} photo=${file.size}B outfit="${outfit}" bg="${bg}" style="${style}" bodyMods=${JSON.stringify(bodyMods)} quality=${quality} picante=${picante} upscale=${upscale} animate=${animate} keepBg=${keepBackground} keepAr=${keepAspectRatio} bodyOnly=${bodyOnly} promptOnly=${promptOnly}`
        );
...
<meta name="apple-mobile-web-app-title" content="MagicVisual">
<meta name="mobile-web-app-capable" content="yes">
<meta name="description" content="Editor de fotos con IA — outfits, body mods, animación">
<link rel="apple-touch-icon" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'%3E%3Crect width='100' height='100' rx='20' fill='%23a855f7'/%3E%3Ctext x='50' y='72' font-size='50' text-anchor='middle' fill='white' font-family='sans-serif' font-weight='bold'%3EMV%3C/text%3E%3C/svg%3E">
<link rel="icon" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'%3E%3Crect width='100' height='100' rx='20' fill='%23a855f7'/%3E%3Ctext x='50' y='72' font-size='50' text-anchor='middle' fill='white' font-family='sans-serif' font-weight='bold'%3EMV%3C/text%3E%3C/svg%3E">
<script>
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    const swCode = \`const C='magicvisual-v3';self.addEventListener('install',e=>self.skipWaiting());self.addEventListener('activate',e=>e.waitUntil(self.clients.claim()));self.addEventListener('fetch',e=>{if(e.request.method!=='GET')return;e.respondWith(fetch(e.request).catch(()=>caches.match(e.request)))});\`;
  ...
  <div class="layout">
    <div>
      <div class="card" style="margin-bottom: 16px;">
        ...
        <div id="faceSeedSection" style="display:none; margin-top:10px; padding:10px; background:rgba(168,85,247,0.1); border:1px solid rgba(168,85,247,0.3); border-radius:8px;">
          <div style="display:flex; align-items:center; justify-content:space-between; gap:8px;">
            <div>
              <strong style="color:var(--accent);">🌱 Cara semilla guardada</strong>
              <p style="font-size:11px; color:var(--text2); margin-top:2px;">Tu rostro se reutiliza automáticamente en cada generación para máximo parecido.</p>
            </div>
            <button class="btn-secondary" id="btnClearSeed" style="font-size:11px; padding:6px 10px; white-space:nowrap;">Quitar semilla</button>
          </div>
        </div>
        <div style="margin-top:8px;">
          <button class="btn-secondary" id="btnSaveSeed" style="font-size:12px; padding:8px 14px; display:none;">
            🌱 Guardar como cara semilla
          </button>
        </div>
      </div>
      ...
      <div class="card" style="margin-bottom: 16px;">
        <h2>2 · Género</h2>
        <div class="style-tabs" style="grid-template-columns: 1fr 1fr;">
          <div class="style-tab active" id="genderMale" data-gender="masculine">
            <div class="name">👨 Hombre</div>
          </div>
          <div class="style-tab" id="genderFemale" data-gender="feminine">
            <div class="name">👩 Mujer</div>
          </div>
        </div>
      </div>
      ...
      <div class="card" style="margin-bottom: 16px;">
        <h2>
          3 · Outfit / Prompt
          <label style="font-size: 11px; font-weight: 500; text-transform: none; letter-spacing: 0; cursor: pointer; display: inline-flex; align-items: center; gap: 4px; color: var(--text2);">
            <input type="checkbox" id="promptOnlyToggle" style="cursor: pointer;"> Solo Prompt
          </label>
          <button class="btn-suggest" id="btnSuggest" title="La IA sugiere 10 outfits para este estilo / modo">✨ Sugerir con IA</button>
        </h2>
        <input type="text" class="outfit-input" id="outfit" placeholder="Ej: red lace lingerie set (o escribí: que esté sentada en el sillón)" autocomplete="off">
        <div class="ideas" id="ideas"></div>
        <div id="promptOnlyHint" style="display:none; margin-top: 8px; padding: 10px; background: rgba(168,85,247,0.1); border: 1px solid rgba(168,85,247,0.3); border-radius: 8px; font-size: 12px; color: var(--accent);">
          <strong>Modo Solo Prompt activado:</strong> La IA respeta la foto, la ropa y el fondo original. Solo aplica lo que escribí arriba. Ej: "que esté sentada en el sillón", "cambiar el fondo a playa", "agregar gafas oscuras".
        </div>
      </div>
      ...
      <div class="card" style="margin-bottom: 16px;">
        ...
        <input type="text" class="outfit-input" id="customBg" placeholder="Fondo personalizado (Ej: terraza de noche, bosque con niebla)" autocomplete="off" style="margin-top:8px; font-size:13px;">
      </div>

      <div class="card" style="margin-bottom: 16px;">
        <h2>8 · Pose / Acción personalizada</h2>
        <input type="text" class="outfit-input" id="customPose" placeholder="Ej: agachado con collar y cadena en la mano, sentado en sillon, mirando al horizonte" autocomplete="off" style="font-size:13px;">
        <p style="font-size:11px; color:var(--text2); margin-top:4px;">La IA prioriza esta pose sobre todo lo demás. Si la dejás vacía, usa pose natural.</p>
      </div>

      <div class="card" style="margin-bottom: 16px;">
        <h2>9 · Animación <span style="font-size:11px; color:var(--text2); text-transform:none; letter-spacing:0;">(opcional, video Z.ai)</span></h2>
        <div class="pill-row cols-3" id="animTabs"></div>
      </div>

      <div style="display:flex; gap:8px;">
        <button class="btn-generate" id="btnGenerate" disabled style="flex:1;">✨ Generar magia</button>
        <button class="btn-generate" id="btnGenerateNoPhoto" style="flex:1; background:linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%);">🎲 Generar sin foto</button>
      </div>

      <div style="margin-top:8px;">
        <button class="btn-generate" id="btnFaceSwapMode" style="width:100%; background:linear-gradient(135deg, #059669 0%, #10b981 100%);">🔄 Cambiar cara (face swap)</button>
      </div>

      <div style="margin-top:8px;">
        <button class="btn-generate" id="btnEnhanceHD" style="width:100%; background:linear-gradient(135deg, #f59e0b 0%, #f97316 100%);">💎 Mejorar HD (upscale + hiperrealista)</button>
      </div>

      <!-- HD Enhancement section -->
      <div id="enhanceHDSection" style="display:none; margin-top:12px; padding:16px; background:rgba(245,158,11,0.08); border:1px solid rgba(245,158,11,0.3); border-radius:12px;">
        <h3 style="font-size:14px; margin-bottom:8px;">💎 Mejora HD Hiperrealista</h3>
        <p style="font-size:12px; color:var(--text2); margin-bottom:12px;">Subí cualquier foto y la mejora: upscale 4x, textura de piel realista, sin recortar.</p>
        <div class="dropzone" id="enhanceDropzone" style="padding:16px; min-height:80px;">
          <div style="font-size:28px; opacity:0.5;">💎</div>
          <div style="font-size:12px; color:var(--text2); margin-top:4px;">Subí la foto a mejorar</div>
          <img id="enhancePreview" style="display:none; max-width:100%; max-height:200px; border-radius:8px; margin-top:8px;" alt="enhance">
          <input type="file" id="enhanceFile" accept="image/*" style="display:none;">
        </div>
        <button class="btn-generate" id="btnDoEnhance" disabled style="width:100%; margin-top:12px; background:linear-gradient(135deg, #f59e0b 0%, #f97316 100%);">💎 ¡Mejorar ahora!</button>
      </div>

      <!-- Face Swap section (hidden by default) -->
      <div id="faceSwapSection" style="display:none; margin-top:12px; padding:16px; background:rgba(16,185,129,0.08); border:1px solid rgba(16,185,129,0.3); border-radius:12px;">
        <h3 style="font-size:14px; margin-bottom:12px;">🔄 Cambiar cara</h3>
        <p style="font-size:12px; color:var(--text2); margin-bottom:12px;">Subí la cara de una persona y el cuerpo de otra. La IA pone la cara en el cuerpo.</p>

        <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px; margin-bottom:12px;">
          <div>
            <label style="font-size:12px; font-weight:600; color:var(--text2); display:block; margin-bottom:4px;">👤 Cara (tu foto)</label>
            <div class="dropzone" id="faceDropzone" style="padding:16px; min-height:80px;">
              <div style="font-size:28px; opacity:0.5;">👤</div>
              <div style="font-size:12px; color:var(--text2); margin-top:4px;">Subí la cara</div>
              <img id="facePreview" style="display:none; max-width:100%; max-height:120px; border-radius:8px; margin-top:8px;" alt="face">
              <input type="file" id="faceFile" accept="image/*" style="display:none;">
            </div>
          </div>
          <div>
            <label style="font-size:12px; font-weight:600; color:var(--text2); display:block; margin-bottom:4px;">💃 Cuerpo (otra foto)</label>
            <div class="dropzone" id="bodyDropzone" style="padding:16px; min-height:80px;">
              <div style="font-size:28px; opacity:0.5;">💃</div>
              <div style="font-size:12px; color:var(--text2); margin-top:4px;">Subí el cuerpo</div>
              <img id="bodyPreview" style="display:none; max-width:100%; max-height:120px; border-radius:8px; margin-top:8px;" alt="body">
              <input type="file" id="bodyFile" accept="image/*" style="display:none;">
            </div>
          </div>
        </div>

        <button class="btn-generate" id="btnDoFaceSwap" disabled style="width:100%; background:linear-gradient(135deg, #059669 0%, #10b981 100%);">🔄 ¡Cambiar cara ahora!</button>
      </div>
...
<script>
  ...
  const state = {
    ...
    photoBodyDesc: null,
    style: 'vanilla',
    gender: 'masculine',  // v3.15: force gender for outfit selection
    outfit: '',
    bg: BACKGROUNDS[0].prompt,
    customBg: '',     // v4.2: custom background input
    customPose: '',   // v4.2: custom pose/action input
    bodyMods: [],
    quality: 'standard',
    picante: 'off',
    animate: 'off',
    ...
    bodyOnly: false,
    promptOnly: false,    // v3.6: when true, keep outfit+bg+identity, only apply the prompt instruction
    faceSeed: null,       // v3.6: saved face photo data URL for consistent identity
    generating: false,
  };
  ...
  // ===== Prompt Only toggle =====
  const promptOnlyToggle = $('promptOnlyToggle');
  const promptOnlyHint = $('promptOnlyHint');
  if (promptOnlyToggle) {
    promptOnlyToggle.onchange = (e) => {
      state.promptOnly = e.target.checked;
      if (e.target.checked) {
        promptOnlyHint.style.display = 'block';
        outfitInput.placeholder = 'Escribí tu instrucción: ej: "que esté sentada en el sillón", "cambiar fondo a playa", "agregar gafas"';
        toast('Modo Solo Prompt: respeta foto+ropa+fondo, solo aplica tu instrucción', 'ok', 4000);
      } else {
        promptOnlyHint.style.display = 'none';
        outfitInput.placeholder = 'Ej: red lace lingerie set (o escribí: que esté sentada en el sillón)';
      }
    };
  }
  ...
  document.querySelectorAll('.style-tab').forEach((tab) => {
    // Skip gender tabs (they have data-gender, not data-style)
    if (tab.dataset.gender) return;
    tab.onclick = () => {
      document.querySelectorAll('.style-tab').forEach((t) => {
        if (!t.dataset.gender) t.classList.remove('active');
      });
  ...
  if (genderMale) {
    genderMale.onclick = () => {
      ...
      toast('Género: Hombre 👨 — ropa masculina forzada', 'ok', 2500);
      renderIdeas();
    };
  }
  if (genderFemale) {
    genderFemale.onclick = () => {
      ...
      toast('Género: Mujer 👩 — ropa femenina forzada', 'ok', 2500);
      renderIdeas();
    };
  }
  ...
  async function handleFile(file) {
    ...
    reader.onload = (e) => {
      ...
      // Show "Save as seed" button after photo is loaded
      const btnSaveSeed = $('btnSaveSeed');
      if (btnSaveSeed && !state.bodyOnly) {
        btnSaveSeed.style.display = 'inline-flex';
      }
  ...
  // ===== Face Seed system =====
  // Saves the current photo's data URL to localStorage as "face seed"
  // The seed is automatically used for face swap in every generation
  const btnSaveSeed = $('btnSaveSeed');
  const btnClearSeed = $('btnClearSeed');
  const faceSeedSection = $('faceSeedSection');

  // Load existing seed on startup
  const existingSeed = localStorage.getItem('mv_face_seed');
  if (existingSeed) {
    state.faceSeed = existingSeed;
    if (faceSeedSection) faceSeedSection.style.display = 'block';
  }

  if (btnSaveSeed) {
    btnSaveSeed.onclick = () => {
      if (!state.photoPreview) {
        toast('Subí una foto primero', 'warn');
        return;
      }
      try {
        localStorage.setItem('mv_face_seed', state.photoPreview);
        state.faceSeed = state.photoPreview;
        if (faceSeedSection) faceSeedSection.style.display = 'block';
        toast('🌱 Cara semilla guardada — se usará en todas las generaciones', 'ok', 4000);
      } catch (e) {
        toast('Error: la foto es muy grande para guardar como semilla', 'err');
      }
    };
  }

  if (btnClearSeed) {
    btnClearSeed.onclick = () => {
      localStorage.removeItem('mv_face_seed');
      state.faceSeed = null;
      if (faceSeedSection) faceSeedSection.style.display = 'none';
      toast('Semilla quitada', 'ok', 2000);
    };
  }
  ...
  // v4.2: Custom background and pose inputs
  const customBgInput = $('customBg');
  const customPoseInput = $('customPose');
  if (customBgInput) customBgInput.oninput = (e) => { state.customBg = e.target.value; };
  if (customPoseInput) customPoseInput.oninput = (e) => { state.customPose = e.target.value; };
  ...
  btnGenerate.onclick = async () => {
    ...
    // If face seed exists, use it as the photo (for better face swap consistency)
    // The seed is the "reference face" that gets used for every generation
    if (state.faceSeed && !state.bodyOnly) {
      // Convert data URL back to File object
      try {
        const arr = state.faceSeed.split(',');
        const mime = arr[0].match(/:(.*?);/)[1];
        const bstr = atob(arr[1]);
        const u8 = new Uint8Array(bstr.length);
        for (let i = 0; i < bstr.length; i++) u8[i] = bstr.charCodeAt(i);
        const seedFile = new File([u8], 'face-seed.jpg', { type: mime });
        formData.append('photo', seedFile);
        toast('🌱 Usando cara semilla para máximo parecido', 'ok', 3000);
      } catch (e) {
        // If seed conversion fails, use the uploaded photo
    ...
    formData.append('customBg', state.customBg || '');
    formData.append('customPose', state.customPose || '');
    ...
    formData.append('promptOnly', state.promptOnly ? 'true' : 'false');
    // If face seed is saved, force face swap even in promptOnly mode (iterative edit)
    formData.append('forceFaceSwap', (state.promptOnly && state.faceSeed && !state.bodyOnly) ? 'true' : 'false');
    ...
    try {
      ...
      while (true) {
        ...
        const events = buf.split(String.fromCharCode(10,10));
        buf = events.pop() || '';
  ...
  // ===== Generate without photo (text-to-image only) =====
  const btnGenerateNoPhoto = $('btnGenerateNoPhoto');
  if (btnGenerateNoPhoto) {
    btnGenerateNoPhoto.onclick = async () => {
      if (state.generating) return;
      if (!state.outfit.trim()) {
        toast('Escribí qué querés que genere', 'warn');
        outfitInput.focus();
        return;
      }
      state.generating = true;
      clearError();
      showProgress(10, 'Generando sin foto (text-to-image)...');
      resultContent.innerHTML = '<div class="empty-state">🎲 Generando imagen desde texto... 5-15 segundos.</div>';

      // Build the full prompt with fashion enhancement + style suffix
      const stylePreset = STYLES[state.style] || STYLES.vanilla;
      const qualityPreset = QUALITY_PRESETS[state.quality || 'standard'];
      const picantePreset = PICANTE_LEVELS[state.picante || 'off'];
      const enhancedOutfit = enhanceOutfitPrompt(sanitizePrompt(state.outfit));
      const bgPart = state.bg ? 'Standing in ' + state.bg + '.' : '';
      const fullPrompt = enhancedOutfit + '. ' + bgPart + ' ' + stylePreset.suffix + ' ' + qualityPreset.suffix + ' ' + picantePreset.suffix;

      try {
        // Use Pollinations directly (no photo, no VLM, no face swap)
        showProgress(30, 'Pollinations generando...');
        const res = await fetch('/api/generate-no-photo', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            prompt: fullPrompt,
  ...
  // ===== Face Swap: two-photo mode =====
  const btnFaceSwapMode = $('btnFaceSwapMode');
  const faceSwapSection = $('faceSwapSection');
  const faceDropzone = $('faceDropzone');
  const bodyDropzone = $('bodyDropzone');
  const faceFileInput = $('faceFile');
  const bodyFileInput = $('bodyFile');
  const facePreviewImg = $('facePreview');
  const bodyPreviewImg = $('bodyPreview');
  const btnDoFaceSwap = $('btnDoFaceSwap');

  let faceSwapFaceFile = null;
  let faceSwapBodyFile = null;

  // Toggle section visibility
  if (btnFaceSwapMode) {
    btnFaceSwapMode.onclick = () => {
      if (faceSwapSection.style.display === 'none') {
        faceSwapSection.style.display = 'block';
        toast('Subí la cara y el cuerpo por separado', 'ok', 3000);
      } else {
  ...
  // Face upload
  if (faceDropzone) {
    faceDropzone.onclick = () => faceFileInput.click();
    faceFileInput.onchange = (e) => {
      if (e.target.files[0]) {
        faceSwapFaceFile = e.target.files[0];
        const reader = new FileReader();
        reader.onload = (ev) => { facePreviewImg.src = ev.target.result; facePreviewImg.style.display = 'block'; };
        reader.readAsDataURL(faceSwapFaceFile);
  ...
  // Body upload
  if (bodyDropzone) {
    bodyDropzone.onclick = () => bodyFileInput.click();
    bodyFileInput.onchange = (e) => {
      if (e.target.files[0]) {
        faceSwapBodyFile = e.target.files[0];
        const reader = new FileReader();
        reader.onload = (ev) => { bodyPreviewImg.src = ev.target.result; bodyPreviewImg.style.display = 'block'; };
        reader.readAsDataURL(faceSwapBodyFile);
        updateFaceSwapButton();
      }
    };
  }

  function updateFaceSwapButton() {
    if (btnDoFaceSwap) {
      btnDoFaceSwap.disabled = !faceSwapFaceFile || !faceSwapBodyFile;
    }
  }

  // Do face swap
  if (btnDoFaceSwap) {
    btnDoFaceSwap.onclick = async () => {
      if (!faceSwapFaceFile || !faceSwapBodyFile) return;
      state.generating = true;
      btnDoFaceSwap.disabled = true;
      clearError();
      showProgress(10, 'Cambiando cara...');
      resultContent.innerHTML = '<div class="empty-state">🔄 Cambiando cara... 10-25 segundos.</div>';

      const fd = new FormData();
      fd.append('face', faceSwapFaceFile);
      fd.append('body', faceSwapBodyFile);

      try {
        const res = await fetch('/api/face-swap', { method: 'POST', body: fd });
        if (!res.ok) {
          const err = await res.text();
          throw new Error('HTTP ' + res.status + ': ' + err);
        }
        ...
        showError('Error: ' + e.message);
        resultContent.innerHTML = '<div class="empty-state">Falló el face swap. Probá de nuevo.</div>';
      } finally {
        state.generating = false;
        btnDoFaceSwap.disabled = false;
      }
    };
  }

  // ===== Enhance HD handler =====
  const btnEnhanceHD = $('btnEnhanceHD');
  const enhanceHDSection = $('enhanceHDSection');
  const enhanceDropzone = $('enhanceDropzone');
  const enhanceFileInput = $('enhanceFile');
  const enhancePreviewImg = $('enhancePreview');
  const btnDoEnhance = $('btnDoEnhance');
  let enhanceFile = null;

  if (btnEnhanceHD) {
    btnEnhanceHD.onclick = () => {
      if (enhanceHDSection.style.display === 'none') {
        enhanceHDSection.style.display = 'block';
        toast('Subí la foto que querés mejorar a HD', 'ok', 3000);
      } else {
        enhanceHDSection.style.display = 'none';
      }
    };
  }

  if (enhanceDropzone) {
    enhanceDropzone.onclick = () => enhanceFileInput.click();
    enhanceFileInput.onchange = (e) => {
      if (e.target.files[0]) {
        enhanceFile = e.target.files[0];
        const reader = new FileReader();
        reader.onload = (ev) => { enhancePreviewImg.src = ev.target.result; enhancePreviewImg.style.display = 'block'; };
  ...
  if (btnDoEnhance) {
    btnDoEnhance.onclick = async () => {
      if (!enhanceFile) return;
      state.generating = true;
      btnDoEnhance.disabled = true;
      clearError();
      showProgress(10, 'Mejorando HD...');
      resultContent.innerHTML = '<div class="empty-state">💎 Mejorando a HD 4x... unos segundos.</div>';

      const fd = new FormData();
      fd.append('photo', enhanceFile);
      ...
      try {
        ...
        showError('Error: ' + e.message);
        resultContent.innerHTML = '<div class="empty-state">Falló la mejora. Probá de nuevo.</div>';
      } finally {
        state.generating = false;
        btnDoEnhance.disabled = false;
      }
  ...
  function handleEvent(data) {
    ...
    if (data.type === 'progress') {
      const stageMap = {
        ...
        analyzed: [25, 'Cuerpo detectado...'],
        parsing: [30, 'Interpretando tu instrucción con IA...'],
        parsed: [35, 'Outfit interpretado...'],
  ...
  function showResult(data) {
    ...
    currentResultDataUrl = dataUrl; // actualizar para el editor de fotos
    ...
    resultContent.innerHTML =
      '<div class="result-wrap">' +
        ...
        '<div class="result-actions">' +
          ...
          '<button class="btn-secondary" id="btnEditResult" style="background:rgba(168,85,247,0.2); border-color:var(--accent);">✏️ Editar resultado</button>' +
          '<button class="btn-secondary" id="btnPhotoEditor" style="background:rgba(245,158,11,0.2); border-color:#f59e0b;">🎨 Editor de foto</button>' +
          '<button class="btn-secondary" onclick="location.reload()">↻ Otra vez</button>' +
        '</div>' +
        '<div id="editResultSection" style="display:none; margin-top:12px; padding:12px; background:rgba(168,85,247,0.08); border:1px solid rgba(168,85,247,0.3); border-radius:8px;">' +
          '<div style="margin-bottom:8px;"><strong style="color:var(--accent);">✏️ Edición iterativa</strong></div>' +
          '<p style="font-size:12px; color:var(--text2); margin-bottom:8px;">Escribí el cambio chiquito que querés. La IA respeta la imagen y solo aplica lo que pedís.</p>' +
          '<input type="text" class="outfit-input" id="editInstruction" placeholder="Ej: poner la mano derecha en la cintura" autocomplete="off" style="width:100%;">' +
          '<button class="btn-generate" id="btnApplyEdit" style="margin-top:8px; width:100%; padding:10px; font-size:14px;">✏️ Aplicar cambio</button>' +
        '</div>' +
        // ===== Photo Editor Canvas =====
        '<div id="photoEditorSection" style="display:none; margin-top:12px; padding:16px; background:rgba(245,158,11,0.06); border:1px solid rgba(245,158,11,0.3); border-radius:12px;">' +
          '<h3 style="font-size:14px; margin-bottom:12px;">🎨 Editor de Foto</h3>' +
          '<div style="position:relative; margin-bottom:12px;">' +
            '<canvas id="editorCanvas" style="max-width:100%; border-radius:8px; display:block;"></canvas>' +
          '</div>' +
          '<div style="display:grid; gap:10px;">' +
            '<div><label style="font-size:11px; color:var(--text2);">Brillo: <span id="brightVal">100%</span></label><br><input type="range" id="sliderBright" min="50" max="150" value="100" style="width:100%;"></div>' +
            '<div><label style="font-size:11px; color:var(--text2);">Contraste: <span id="contrastVal">100%</span></label><br><input type="range" id="sliderContrast" min="50" max="200" value="100" style="width:100%;"></div>' +
            '<div><label style="font-size:11px; color:var(--text2);">Suavizar piel: <span id="smoothVal">0%</span></label><br><input type="range" id="sliderSmooth" min="0" max="100" value="0" style="width:100%;"></div>' +
          '</div>' +
          '<div style="display:flex; gap:6px; margin-top:10px; flex-wrap:wrap;">' +
            '<button class="btn-secondary" id="filterWarm" style="font-size:11px; padding:6px 10px;">☀️ Cálido</button>' +
            '<button class="btn-secondary" id="filterCold" style="font-size:11px; padding:6px 10px;">❄️ Frío</button>' +
            '<button class="btn-secondary" id="filterVintage" style="font-size:11px; padding:6px 10px;">📷 Vintage</button>' +
            '<button class="btn-secondary" id="filterReset" style="font-size:11px; padding:6px 10px;">↺ Reset</button>' +
          '</div>' +
          '<button class="btn-generate" id="btnSaveEdit" style="margin-top:12px; width:100%; background:linear-gradient(135deg, #f59e0b 0%, #f97316 100%);">💾 Guardar edición final</button>' +
        '</div>' +
      '</div>';

    // ===== Edit Result: iterative editing =====
    const btnEditResult = $('btnEditResult');
    const editResultSection = $('editResultSection');
    const btnApplyEdit = $('btnApplyEdit');
    const editInstruction = $('editInstruction');

    if (btnEditResult) {
      btnEditResult.onclick = () => {
        if (editResultSection.style.display === 'none') {
          editResultSection.style.display = 'block';
          editInstruction.focus();
          toast('Escribí el cambio que querás y clickeá Aplicar', 'ok', 3000);
        } else {
          editResultSection.style.display = 'none';
        }
      };
    }
    ...
    if (btnApplyEdit) {
      btnApplyEdit.onclick = async () => {
        ...
        if (!instruction) {
          toast('Escribí qué cambio querás hacer', 'warn');
          return;
        }

        // Convert the result image data URL to a File object
        try {
          ...
          const u8 = new Uint8Array(bstr.length);
          for (let i = 0; i < bstr.length; i++) u8[i] = bstr.charCodeAt(i);
          const resultFile = new File([u8], 'edit-base.jpg', { type: mime });

          // Set up state for iterative edit
          state.photoFile = resultFile;
          state.photoPreview = dataUrl;
          state.outfit = instruction;
          state.promptOnly = true; // force prompt-only mode
          state.keepBackground = true; // keep the background from the result
          outfitInput.value = instruction;

          // Show the result image as the new preview
          preview.src = dataUrl;
          preview.style.display = 'block';
          photoInfo.style.display = 'block';
          photoInfo.innerHTML = '<span style="color:var(--accent);">✏️ Editando resultado anterior</span>';

          // Hide the edit section
          editResultSection.style.display = 'none';

          // Auto-generate
          toast('✏️ Aplicando cambio: ' + instruction.slice(0, 40), 'ok', 3000);
          generateBtn.click();
        } catch (e) {
          toast('Error al preparar la edición', 'err');
        }
      };
    }
  }

  // ===== Photo Editor with Canvas =====
  // Usar event delegation porque el botón se crea dinámicamente
  let editorImg = null;
  let editorCanvas = null;
  let editorCtx = null;
  let editorFilter = { brightness: 100, contrast: 100, smooth: 0, preset: '' };
  let currentResultDataUrl = null; // se actualiza cada vez que se muestra un resultado

  document.addEventListener('click', (e) => {
    if (e.target && e.target.id === 'btnPhotoEditor') {
      const photoEditorSection = $('photoEditorSection');
      if (!photoEditorSection) return;
      if (photoEditorSection.style.display === 'none' || photoEditorSection.style.display === '') {
        ...
        img.src = currentResultDataUrl || dataUrl;
        toast('Editor abierto — ajustá los sliders', 'ok', 2500);
      } else {
  ...
  function renderEditor() {
    if (!editorCtx || !editorImg || !editorCanvas) return;
    const w = editorCanvas.width;
    const h = editorCanvas.height;
    
    // Apply CSS filter
    const filterStr = 'brightness(' + editorFilter.brightness + '%) contrast(' + editorFilter.contrast + '%)' +
      (editorFilter.preset === 'warm' ? ' sepia(0.2) saturate(1.4)' : '') +
      (editorFilter.preset === 'cold' ? ' hue-rotate(10deg) saturate(1.1)' : '') +
      (editorFilter.preset === 'vintage' ? ' sepia(0.3) contrast(0.9) brightness(1.05)' : '');
    
    editorCtx.filter = filterStr;
    editorCtx.drawImage(editorImg, 0, 0, w, h);
    
    // Skin glow — overlay blurred copy with screen blend
    if (editorFilter.smooth > 0) {
      const opacity = editorFilter.smooth / 100 * 0.4;
      editorCtx.filter = 'blur(' + (2 + editorFilter.smooth / 20) + 'px) brightness(1.1)';
      editorCtx.globalAlpha = opacity;
      editorCtx.globalCompositeOperation = 'screen';
      editorCtx.drawImage(editorImg, 0, 0, w, h);
    ...
    // Vintage grain
    if (editorFilter.preset === 'vintage') {
      const grainData = editorCtx.getImageData(0, 0, w, h);
      ...
      editorCtx.putImageData(grainData, 0, 0);
    }
  }

  // Sliders — event delegation (same problem as the button)
  document.addEventListener('input', (e) => {
    if (!e.target) return;
    if (e.target.id === 'sliderBright') {
      editorFilter.brightness = parseInt(e.target.value);
      const v = $('brightVal'); if (v) v.textContent = editorFilter.brightness + '%';
      renderEditor();
    } else if (e.target.id === 'sliderContrast') {
      editorFilter.contrast = parseInt(e.target.value);
      const v = $('contrastVal'); if (v) v.textContent = editorFilter.contrast + '%';
      renderEditor();
    } else if (e.target.id === 'sliderSmooth') {
      editorFilter.smooth = parseInt(e.target.value);
      const v = $('smoothVal'); if (v) v.textContent = editorFilter.smooth + '%';
      renderEditor();
    }
  });
  ...
  document.addEventListener('click', (e) => {
    if (!e.target) return;
    if (e.target.id === 'filterWarm') { editorFilter.preset = 'warm'; renderEditor(); toast('Filtro cálido', 'ok', 1500); }
    else if (e.target.id === 'filterCold') { editorFilter.preset = 'cold'; renderEditor(); toast('Filtro frío', 'ok', 1500); }
    else if (e.target.id === 'filterVintage') { editorFilter.preset = 'vintage'; renderEditor(); toast('Filtro vintage', 'ok', 1500); }
    else if (e.target.id === 'filterReset') {
      editorFilter = { brightness: 100, contrast: 100, smooth: 0, preset: '' };
      const sb = $('sliderBright'); if (sb) sb.value = 100;
      const sc = $('sliderContrast'); if (sc) sc.value = 100;
      const ss = $('sliderSmooth'); if (ss) ss.value = 0;
      const bv = $('brightVal'); if (bv) bv.textContent = '100%';
      const cv = $('contrastVal'); if (cv) cv.textContent = '100%';
      const sv = $('smoothVal'); if (sv) sv.textContent = '0%';
      renderEditor();
      toast('Filtros reseteados', 'ok', 1500);
    }
  });

  // Save edited image — event delegation
  document.addEventListener('click', (e) => {
    if (!e.target || e.target.id !== 'btnSaveEdit') return;
    if (!editorCanvas) { toast('No hay imagen para guardar', 'err'); return; }
    const editedDataUrl = editorCanvas.toDataURL('image/jpeg', 0.95);
    const resultImg = document.querySelector('.result-img');
    if (resultImg) resultImg.src = editedDataUrl;
    const dlLink = document.querySelector('.result-actions a[download]');
    if (dlLink) {
      dlLink.href = editedDataUrl;
      dlLink.download = 'magicvisual-edited-' + Date.now() + '.jpg';
    }
    saveToHistory({ result: editedDataUrl, bodyDesc: 'Edited with photo editor', animationTaskId: null });
    toast('¡Edición guardada! 💾', 'ok', 2500);
    const pes = $('photoEditorSection');
  ...
  function setHistory(arr) {
    localStorage.setItem('mv_history', JSON.stringify(arr.slice(0, 100)));
    renderHistory();
  }
  async function saveToHistory(data) {
    // Guardar imagen COMPLETA con buena calidad
    // El thumbnail es solo para mostrar en el grid, pero fullImg es HD
    try {
      const arr = getHistory();
      const imgArr = data.result.split(',');
      const mime = imgArr[0].match(/:(.*?);/)[1];
      const bstr = atob(imgArr[1]);
      const u8 = new Uint8Array(bstr.length);
      for (let i = 0; i < bstr.length; i++) u8[i] = bstr.charCodeAt(i);
      const blob = new Blob([u8], { type: mime });
      const bmp = await createImageBitmap(blob);
      
      // Thumbnail más grande para el grid (512x512 en vez de 256x256)
      ...
      arr.unshift({
        img: thumb,           // 512x512 para el grid
        fullImg: data.result, // IMAGEN COMPLETA en HD para descargar
        ts: Date.now(),
        outfit: state.outfit,
        style: state.style,
        bodyDesc: data.bodyDesc,
        animationTaskId: data.animationTaskId || null,
      });
      
      // Guardar hasta 100 imágenes (era 24)
      // localStorage tiene límite de ~5MB, así que intentamos con fullImg
      // si falla, guardamos solo thumbnails más grandes
      try {
        localStorage.setItem('mv_history', JSON.stringify(arr.slice(0, 100)));
      } catch (e) {
        // Quota exceeded — guardar solo thumbnails (sin fullImg) pero permitir 100
        try {
          const light = arr.slice(0, 100).map(item => ({ ...item, fullImg: undefined }));
          localStorage.setItem('mv_history', JSON.stringify(light));
          console.warn('[History] Saved thumbnails only (quota exceeded)');
        } catch (e2) {
          // Si todavía falla, guardar menos (50)
          const light = arr.slice(0, 50).map(item => ({ ...item, fullImg: undefined }));
          localStorage.setItem('mv_history', JSON.stringify(light));
          console.warn('[History] Saved 50 thumbnails only');
        }
      }
      renderHistory();
    } catch (e) {
  ...
  function renderHistory() {
    ...
    historyContent.querySelectorAll('.history-item').forEach((el) => {
      el.onclick = (e) => {
        ...
        const arr = getHistory();
        showResult({
          result: arr[i].fullImg || arr[i].img, // use full image if available
          ...
          animationTaskId: arr[i].animationTaskId || null,
        });
      };
    });
  }
